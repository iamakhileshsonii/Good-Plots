...
const AwaitingAppointmentCard = ({ appointment, appointmentId }) => {
  useEffect(() => {
    initFlowbite();
  }, []);
  ...
  useEffect(() => {
    async function getCardData() {
      ...
      const appointmentClient = await getPropertyOwnerDetails(
      ...
      setPropertyOwner(appointmentClient);

      if (appointmentClient?.role === "0") {
        setUserRole("Buyer/ Seller");
      } else if (appointmentClient?.role === "1") {
        setUserRole("Broker");
      } else if (appointmentClient?.role === "2") {
  ...
  async function acceptAppointment(appointment_id) {
    const response = await acceptAppointmentApi(appointment_id);
    if (!response) {
      window.alert("Something went wrong");
    }
  }
  ...
  return (
    <div class="px-2 bg-white border border-gray-200 rounded-lg shadow dark:bg-gray-800 dark:border-gray-700">
      <div class="flex flex-col  items-center pb-3">
        <div class="flex flex-row gap-4 py-2 px-1 items-center ">
          <img
          ...
          <div class="flex justify-end pr-2 pt-1">
            ...
            >
              <ul class="py-2" aria-labelledby="dropdownButton">
                ...
                <li onClick={() => acceptAppointment(appointment?._id)}>
                  <a class="block px-4 py-2 text-sm text-gray-700 hover:bg-gray-100 dark:hover:bg-gray-600 dark:text-gray-200 dark:hover:text-white">
                    Accept
                  </a>
                </li>
      ...
      <div
        ...
        tabindex="-1"
        aria-hidden="true"
        class="hidden overflow-y-auto overflow-x-hidden fixed top-0 right-0 left-0 z-50 justify-center items-center w-full md:inset-0 h-[calc(100%-1rem)] max-h-full overscroll-y-contain"
      >
        <div class="relative p-4 w-1/4">
          <div class="relative bg-white rounded-lg shadow dark:bg-gray-700">
            ...
            <div class="p-4 md:p-5 space-y-4">
              <div>
                <img
                  src={propertyData?.propertyData[0]?.photos.siteView || ""}
                  alt=""
            ...
            <div class="flex justify-center items-center gap-8 p-4 md:p-5 border-t border-gray-200 rounded-b dark:border-gray-600">
              <svg
              ...
              <svg
                ...
                strokeWidth="1.5"
                stroke="currentColor"
                class="size-6 text-red hover:cursor-pointer"
                data-modal-hide="static-modal"
              >
                <path
                  strokeLinecap="round"
              ...
              <svg
                ...
                strokeWidth="1.5"
                stroke="currentColor"
                class="size-6 text-green hover:cursor-pointer"
...
export default AwaitingAppointmentCard;
