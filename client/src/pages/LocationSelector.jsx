...
const LocationSelector = ({ user }) => {
  ...
  const { assignPincodeToBroker, brokerloading, brokerError } =
    useAssignBrokerPincode();
  const { assignPincodeToLawyer, lawyerLoading, lawyerError } =
    useAssignLawyerPincode();
  const { assignedPincodes } = useAssignedPincodes();
  const { brokerPins } = useGetBrokerAssignedPins();
  const { lawyerPins } = useGetLawyerAssignedPins();

  useEffect(() => {
    if (user?.role === "1" || user?.role === 1) {
      console.log("BROKER ASSIGNED PINS: ", brokerPins);
    } else if (user?.role === "3" || user?.role === 3) {
      console.log("LAWYER ASSIGNED PINS: ", lawyerPins);
    } else {
      console.log("Nothing to do");
    }
  }, [brokerPins, lawyerPins]);
  ...
  const handlePincodeSelection = (event, pincode) => {
    ...
    if (isChecked) {
      setSelectedPincodes((prev) => [...prev, pincode]);
    } else {
      setSelectedPincodes((prev) => prev.filter((pin) => pin !== pincode));
    }
  };

  // Handle Assign Pincode to Broker
  const handleAssignPinToBroker = async (e) => {
    e.preventDefault();
    console.log("Assigned Pincode to Broker");
    const result = await assignPincodeToBroker(selectedPincodes, user);
    if (result) {
      console.log("Pincodes assigned to broker successfully:", result);
    }
  };
  ...
  //Fetch Assigned Pincodes for BROKER
  const fetchBrokerAssignedPins = async () => {
    console.log("BROKER ASSIGNED PINS: ", brokerPins);
  };

  //Fetch Assigned Pincodes for LAWYER
  const fetchLawyerAssignedPins = async () => {
    console.log("BROKER ASSIGNED PINS: ", lawyerPins);
  };
  ...
  return (
    <div className="p-1">
        ...
        (pincodes.length > 0 ? (
          <div className="overflow-x-auto h-96 overflow-y-scroll">
            <table className="min-w-full bg-white border border-gray-200">
              ...
              <tbody>
                {/* {pincodes.map((place) => {
                  const isAssigned = assignedPincodes.includes(
                  ...
                  return (
                    <tr key={place.value} className="border-t border-gray-200">
      ...
      {user?.role === "3" || user?.role === 3 ? (
        ...
        <button
          onClick={handleAssignPinToBroker}
          disabled={loading || selectedPincodes.length === 0}
          className="mt-4 px-6 py-2 text-white bg-blue-500 rounded-md hover:bg-blue-600 disabled:opacity-50"
        >
          {loading ? "Assigning..." : "Assign Selected  Pincodes to Broker"}
        </button>
      )}
